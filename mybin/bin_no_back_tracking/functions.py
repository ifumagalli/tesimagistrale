...
class CostFunctional(object):
	def __init__(self,nu,solver,adjoint_solver=None,penalty_coeffs=[1.0,1.0],**kwargs):
		...
		# else:
			# coeffN *= 2
		self._coeffspace = FunctionSpace(UnitInterval(coeffN),'CG',5)
		self._coeffVspace = VectorFunctionSpace(UnitSquare(coeffN,coeffN),'CG',5)
	...
	# END DEBUG
		...
		trace_form = dot(grad(dq_uzs),grad(phi))*dx + Constant('100')*dq_uzs * phi * ds(0)
			# ??? ha senso questa sorta di rilevamento?
		trace_rhs = -0.5*inner(grad(self._u)*DTq,grad(self._u)*DTq) * phi * ds(0) \
				- dot(grad(self._u)*DTq*DTq_invT*n, \
