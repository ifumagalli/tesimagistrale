...
Nq = 20
I = UnitIntervalMesh(Nq) # mesh for controls
Nup = Nq
...
u = project(u0,V)
#u.vector()[:] = array([u0(x) for x in Omega.coordinates()])
	# ??? un modo migliore per inizializzare?
#q = q0
Vbar_form = abs(q)*dx	
...
inflow = DirichletBC(V,u_in,lambda x, on_boundary : (not Gamma_moving.inside(x,on_boundary)) and x[0] < DOLFIN_EPS)
# noslip2 = DirichletBC(V,Expression(('0','0')),lambda x, on_boundary : Gamma_still.inside(x,on_boundary) and x[1] > 1-DOLFIN_EPS)
symmetry = DirichletBC(V.sub(1),Expression('0'),lambda x, on_boundary : (not Gamma_moving.inside(x,on_boundary)) and x[1] > 1-DOLFIN_EPS)
	# ??? DA VERIFICARE
...
J = CostFunctional(nu,f,bc,J_form,solver,grad_form=gradJ_form)
