...
class CostFunctional(object):
	def __init__(self,nu,solver,adjoint_solver=None,penalty_coeffs=[0.0,0.0],**kwargs):
		...
		print "Penalty coefficients = ", penalty_coeffs
...
#		trace_bc = DirichletBC(P,Constant('0.0'),lambda x,on_boundary : on_boundary and ( near(x[0],0.0) or near(x[0],1.0) ))
		# Da Manzoni, p.75
		trace_form = dot(grad(dq_uzs),grad(phi))*dx + (1+sum(self._penalty_coeffs))*dq_uzs*phi*dx
		trace_rhs = 0.5*inner(grad(self._u)*DTq_inv,grad(self._u)*DTq_inv) * phi * ds(0) \
				+ dot(grad(self._u)*DTq_inv*DTq_invT*n, \
				 (grad(self._u)-self._nu*grad(self._z))*DTq_inv*DTq_invT*n) * phi * ds(0) \
				+ self._penalty_coeffs[0]*Constant(assemble(q * dx)-self._Vbar) *phi * ds(0) \
				+ self._penalty_coeffs[1]*D4(q) * phi * ds(0)
				 # + self._s*DTq_invT*n) * phi * ds(0)
...
#@#@#@					bOmega.entity_map(0)[parent_vertex[Q.dofmap().vertex_to_dof_map(I)[i]]]]]
		print "penalty", self._penalty_coeffs
...
#		value = project(func_value,q.function_space())
		value = project(func_value_from_state_and_adjoint,q.function_space())
		plot(value,title="value restituito da gradient",key="value_grad")
		return value
